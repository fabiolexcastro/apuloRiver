...
graw.ftr <- autoplot(map.latlon) + 
  geom_tile(data = rraw.ftr.tbl.avg, aes(x = x, y = y, fill = value)) + 
...
graw.bsl
ggsave(plot = graw.ftr, filename = './png/maps/temp_raw-ftr_cundinamarca.jpg', units = 'in', width = 9, height = 7, dpi = 300)


# Temperatura raw future --------------------------------------------------
...
ginterpolate <- autoplot(map.latlon) + 
  ...
  geom_sf(data = st_as_sf(cndn), fill = NA, col = 'grey30', inherit.aes = FALSE) +
  coord_sf(xlim = c(-74.88, -73.04), ylim = c(3.73, 5.83)) +
  labs(x = 'Lon', y = 'Lat', fill = 'Temperatura interpolada (°C)') +
  theme_minimal() +
  theme(legend.position = 'bottom', 
        legend.key.width = unit(2.5, 'line'),
        axis.text.y = element_text(angle = 90, hjust = 0.5),
